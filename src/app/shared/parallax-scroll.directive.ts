import {AfterViewInit, Directive, ElementRef, Input, OnDestroy, Renderer2, ViewContainerRef} from '@angular/core';
...
export class ParallaxScrollDirective implements AfterViewInit, OnDestroy {
  ...
  constructor(private viewContainerRef: ViewContainerRef, private elementRef: ElementRef, private renderer: Renderer2) {
    this.htmlElement = this.viewContainerRef.element.nativeElement;
  }

  ngAfterViewInit(): void {
    this.scrollEventSubscription = fromEvent(window, 'scroll').subscribe((scrollEvent: UIEvent) => {
        // console.log(scrollEvent);
        // console.log(scrollEvent.view.pageYOffset);
        let backgroundOffset = scrollEvent.view.pageYOffset * 0.5;
        this.renderer.setStyle(this.htmlElement, 'background-position-y', '-' + backgroundOffset + 'px');
      }
    );
